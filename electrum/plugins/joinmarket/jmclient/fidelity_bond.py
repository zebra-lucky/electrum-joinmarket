...
def get_cert_msg(cert_pub, cert_expiry):
    return (b'fidelity-bond-cert|' + cert_pub +
            b'|' + str(cert_expiry).encode('ascii'))


def get_ascii_cert_msg(cert_pub, cert_expiry):
    return (b'fidelity-bond-cert|' + binascii.hexlify(cert_pub) +
            b'|' + str(cert_expiry).encode('ascii'))
...
class FidelityBondProof:
    # nick_sig + cert_sig + cert_pubkey + cert_expiry + utxo_pubkey +
    # txid + vout + timelock
    # 72       + 72       + 33          + 2           + 33          +
    # 32   + 4    + 4 = 252 bytes
    ...
    def parse_and_verify_proof_msg(cls, maker_nick, taker_nick, data):
        ...
        if not cls._verify_signature(proof.nick_msg, signature,
                                     proof.cert_pub):
            raise ValueError("nick sig does not verify")
        if (not cls._verify_signature(cert_msg, proof.cert_sig, proof.utxo_pub)
                and not cls._verify_signature(ascii_cert_msg, proof.cert_sig,
                                              proof.utxo_pub)):
