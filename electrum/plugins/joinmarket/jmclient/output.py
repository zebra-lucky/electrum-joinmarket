...
general_custom_change_warning = """You are attempting to send change to a
custom change address. Change outputs are usually directly linkable to
...
sweep_custom_change_warning = ("Custom change cannot be set while doing"
                               " a sweep (zero amount).")


def fmt_utxos(utxos, jmman, prefix=''):
    output = []
    for u in utxos:
        utxo_str = '{}{} - {}'.format(
            prefix, fmt_utxo(u), fmt_tx_data(utxos[u], jmman))
...
def fmt_tx_data(tx_data, jmman):
    int_path = jmman.jmw.script_to_path(tx_data['script'])
    str_path = convert_bip32_intpath_to_strpath(int_path)
    addr = jmman.jmw.script_to_addr(tx_data['script'])
    return 'path: {}, address: {} , value: {}'.format(str_path, addr,
                                                      tx_data['value'])


def generate_podle_error_string(priv_utxo_pairs, to, ts, jmman, cjamount,
    ...
    if len(priv_utxo_pairs) == 0:
        ...
        for p, u in priv_utxo_pairs:
            errmsg += (fmt_utxo(u) + "\n")
    errmsg += ("2: Utxos that have less than " +
               str(taker_utxo_age) + " confirmations:\n")
    if len(to) == 0:
        errmsg += ("None\n")
    else:
        for t in to:
    ...
    errmsg += ("If you cannot source a utxo from your spending mixdepth"
               " according to these rules, use the tool add-utxo.py to"
               " source a utxo from another mixdepth or a "
               "utxo external to your joinmarket wallet. Read the help "
               "with 'python add-utxo.py --help'\n\n")
    errmsg += ("***\nFor reference, here are the utxos in your wallet:\n")

    for md, utxos in jmman.jmw.get_utxos_by_mixdepth().items():
        if not utxos:
            continue
        errmsg += ("\nmixdepth {}:\n{}".format(
            md, fmt_utxos(utxos, jmman, prefix='    ')))
