...
from ..jmbase import utxostr_to_utxo, utxo_to_utxostr, hextobin, bintohex
...
class PoDLE(object):
    ...
    def __init__(self,
        ...
        if not priv:
            ...
            self.P = podle_PublicKey(self.priv.get_public_key_bytes())
        if P2:
    ...
    def get_commitment(self):
        ...
        self.commitment = hashlib.sha256(
            self.P2.get_public_key_bytes()).digest()
    ...
    def generate_podle(self, index=0, k=None):
        ...
        self.e = hashlib.sha256(b''.join(
            [x.get_public_key_bytes() for x in [KG, KJ, self.P, self.P2]])
        ).digest()
        priv_int, e_int = (int.from_bytes(x, byteorder="big")
                           for x in [self.priv.get_secret_bytes(), self.e])
    ...
    def reveal(self):
        ...
        return {'used': self.used,
                'utxo': self.u,
                'P': self.P.get_public_key_bytes(),
                'P2': self.P2.get_public_key_bytes(),
    ...
    def verify(self, commitment, index_range):
        ...
        for J in [getNUMS(i) for i in index_range]:
            ...
            sG = sig_priv.get_public_key_bytes()
            sJ = multiply(int.from_bytes(self.s, byteorder='big'),
                          J.get_public_key_bytes())
            e_int = int.from_bytes(self.e, byteorder="big")
            minus_e = (-e_int % N).to_bytes(32, byteorder="big")
            minus_e_P = multiply(int.from_bytes(minus_e, byteorder='big'),
                                 self.P.get_public_key_bytes())
            minus_e_P2 = multiply(int.from_bytes(minus_e, byteorder='big'),
                                  self.P2.get_public_key_bytes())
            KGser = add_pubkeys([sG, minus_e_P])
            KJser = add_pubkeys([sJ, minus_e_P2])
            # check 2: e =?= H(K_G || K_J || P || P2)
            e_check = hashlib.sha256(
                KGser + KJser + self.P.get_public_key_bytes() +
                self.P2.get_public_key_bytes()).digest()
            if e_check == self.e:
                return True
...
def verify_all_NUMS(write=False):
    ...
    nums_points = {}
    for i in range(256):
        nums_points[i] = bintohex(getNUMS(i).get_public_key_bytes())
    assert nums_points == precomp_NUMS, ("Precomputed NUMS points"
                                         " are not valid!")
...
def getP2(priv, nums_pt):
    ...
    """
    priv_raw = priv.secret_scalar
    return multiply(priv_raw,
                    nums_pt.get_public_key_bytes(),
...
def write_podle(jmman, used, external):
    """Update persisted commitment data"""
    used_list = [bintohex(x) for x in used]
    external_dict = external_dict_to_file(external)
    jmman.jmw.set_jm_commitments(used=used_list, external=external_dict)


def read_podle(jmman):
    """Returns used commitment list and external commitments dict struct"""
    c = jmman.jmw.get_jm_commitments()
    used = ([hextobin(x) for x in c["used"]] if "used" in c
            else [])
    external = (external_dict_from_file(c["external"]) if "external" in c
                else {})
    return (used, external)


def get_podle_commitments(jmman):
    ...
    """
    return read_podle(jmman)


def add_external_commitments(jmman, ecs):
    """To allow external functions to add
    PoDLE commitments that were calculated elsewhere.
    """
    update_commitments(jmman, external_to_add=ecs)


def update_commitments(jmman, commitment=None,
    ...
    """
    commitments, external = read_podle(jmman)
    if commitment:
    ...
    write_podle(jmman, commitments, external)


def get_podle_tries(jmman, utxo, priv=None, max_tries=1, external=False):
    used_commitments, external_commitments = get_podle_commitments(jmman)

    if external:
        if utxo in external_commitments:
            ec = external_commitments[utxo]
            # use as many as were provided in the file,
            # up to a max of max_tries
...
def generate_podle(jmman, priv_utxo_pairs, max_tries=1, allow_external=None,
                   k=None):
    ...
    for priv, utxo in priv_utxo_pairs:
        tries = get_podle_tries(jmman, utxo, priv, max_tries)
        if tries >= max_tries:
            continue
