...
def parse_schedule_line(schedule, sl):
    sl = sl.decode('utf-8')
    if sl.startswith("#"):
        return (True, None)
    try:
    ...
    if destaddr not in ["INTERNAL", "addrask"]:
        success = is_address(destaddr)
        if not success:
            return (False, "Invalid address: " + destaddr)
...
def get_tumble_schedule(options, destaddrs, mixdepth_balance_dict,
    ...
    any other taker algo; it interprets floats as fractions and integers
    as satoshis, and zero as sweep (as before).
    Args:
    * options - as specified in scripts/tumbler.py and taken from
      cli_options.py
    * destaddrs - a list of valid address strings for the destination of funds
    * mixdepth_balance_dict - a dict mapping mixdepths to balances, note zero
      balances are not included.
    * max_mixdepth_in_wallet - this is actually the highest mixdepth in this
      JM wallet (so 4 actually means 5 mixdepths in wallet; we do not consider
      the possibility of a wallet not having a 0 mixdepth).
      The default value 4 is almost always used.
    """

    def lower_bounded_int(thelist, lowerbound):
        return [int(lb) if int(lb) >= lowerbound else lowerbound
                for lb in thelist]
    ...
    tx_list = []
    # stage 1 coinjoins, which sweep the entire mixdepth without
    # creating change
    sweep_mixdepths = []
    ...
    for mixdepth, wait, makercount in zip(sweep_mixdepths, waits, makercounts):
        ...
        # can't simply check each filled mixdepth and add 1, because the
        # sequencing of these joins may sweep up coins that have already
        # been swept.
        # so we keep track with a binary flag for every mixdepth during
        # this sequence.
        ...
        tx_list.append(tx)
    try:
        lowest_nonempty_mixdepth = min([x for x, y
                                        in nonempty_mixdepths.items()
                                        if y == 1])
    except ValueError:
        raise ScheduleGenerationErrorNoFunds
    # stage 2 coinjoins, which create a number of random-amount coinjoins
    # from each mixdepth
    for m, txcount in enumerate(txcounts):
        ...
        makercounts = lower_bounded_int(makercounts, options['minmakercount'])
        do_rounds = [random.random() < options['rounding_chance']
                     for _ in range(txcount)]
        for amount_fraction, wait, makercount, do_round in zip(
                amount_fractions, waits, makercounts, do_rounds):
            rounding = NO_ROUNDING
            if do_round:
                weight_sum = 1.0*sum(options['rounding_sigfig_weights'])
                weight_prob = [a/weight_sum
                               for a in options['rounding_sigfig_weights']]
                rounding = rand_weighted_choice(
                    len(weight_prob), weight_prob
                ) + 1
            tx = {
            ...
            tx_list.append(tx)
        # reset the final amt_frac to zero, as it's the last one
        # for this mixdepth:
...
def tweak_tumble_schedule(jmman, options, schedule, last_completed,
                          destaddrs=[]):
    ...
    if altered[1] == 0:
        ...
        if new_n_cp < jmman.jmconf.minimum_makers:
            new_n_cp = jmman.jmconf.minimum_makers
        altered[2] = new_n_cp
    if not altered[1] == 0:
        # For non-sweeps, there's a fractional amount (tumbler).
        # Increasing or decreasing the amount could improve the odds of
        # success, since it depends on liquidity and minsizes, so we tweak
        # in both directions randomly.
        # Strategy:
        # 1. calculate the total percentage remaining in the mixdepth.
        # 2. calculate the number remaining incl. sweep.
        # 3. Re-use 'getamountfracs' algo for this reduced number, then
        # scale it to the number remaining.
        ...
        already_done = this_mixdepth_entries[
            :this_mixdepth_entries.index(altered)
        ]
        ...
        for i, j in enumerate(range(last_completed + 1, last_completed + 1 +
                                    len(tobedone))):
