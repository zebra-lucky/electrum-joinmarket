...
def get_random_bytes(num_bytes, cryptographically_secure=False):
    if cryptographically_secure:
        ...
        generator = random
    return bytes(bytearray((generator.randrange(256)
...
def select_greedy(unspent, value):
    ...
    raise NotEnoughFundsException(original_value, picked_sum)
...
def weighted_order_choose(jmman, orders, n):
    """
    ...
    jmman.logger.debug('phi=' + str(phi) + ' weights = ' + str(weight))
...
def cheapest_order_choose(jmman, orders, n):
    """
...
def fidelity_bond_weighted_order_choose(jmman, orders, n):
    """
    choose orders based on fidelity bond for improved sybil resistance

    * with probability `bondless_makers_allowance`: will revert to previous
      default order choose (random_under_max_order_choose)
    * with probability `1 - bondless_makers_allowance`: if there are no bond
      offerings, revert to previous default as above. If there are, choose
      randomly from those, with weighting being the fidelity bond values.
    """

    if random.random() < jmman.jmconf.bondless_makers_allowance:
...
def choose_orders(jmman, offers, cj_amount, n, chooseOrdersBy,
                  ignored_makers=None, pick=False,
    ...
    jmman.logger.debug('considered orders = \n' +
                       '\n'.join([str(o) for o in orders_fees]))
    total_cj_fee = 0
    chosen_orders = []
    for i in range(n):
        chosen_order, chosen_fee = chooseOrdersBy(jmman, orders_fees, n)
...
def choose_sweep_orders(jmman, offers,
    ...
    mychange = totalin - cjamount - total_txfee -
        sum(absfee) - sum(relfee*cjamount)
    ...
    def calc_zero_change_cj_amount(ordercombo):
        ...
        for order in ordercombo:
            ...
            elif order['ordertype'] in ['sw0reloffer', 'swreloffer',
                                        'reloffer']:
                sumrelfee += Decimal(order['cjfee'])
            # this is unreachable since calc_cj_fee must already have
            # been called
    ...
    jmman.logger.debug('choosing sweep orders for total_input_value = ' +
                       str(total_input_value) + ' n=' + str(n))
    ...
    # while we do not know the exact cj value yet, we can approximate
    # a ceiling:
    offers = [o for o in offers
              if o['maxsize'] > (total_input_value - total_txfee)]

    jmman.logger.debug('orderlist = \n' + '\n'.join([str(o) for o in offers]))
    ...
    while len(chosen_orders) < n:
        for i in range(n - len(chosen_orders)):
            if len(orders_fees) < n - len(chosen_orders):
                jmman.logger.debug('ERROR not enough liquidity'
                                   ' in the orderbook')
                # TODO handle not enough liquidity better, maybe an Exception
                return None, 0, 0
            chosen_order, chosen_fee = chooseOrdersBy(jmman, orders_fees, n)
            jmman.logger.debug('chosen = ' + str(chosen_order))
            # remove all orders from that same counterparty
            orders_fees = [
                o
                for o in orders_fees
        ...
        for c in list(chosen_orders):
            ...
            if cj_amount > maxsize or cj_amount < minsize:
                jmman.logger.info(f'cj_amount={cj_amount} do not pass minsize'
                                  f'={minsize}/maxsize={maxsize} limits')
                chosen_orders.remove(c)
    jmman.logger.debug('chosen orders = \n' +
                       '\n'.join([str(o) for o in chosen_orders]))
    result = dict([(o['counterparty'], o) for o in chosen_orders])
    jmman.logger.debug('cj amount = ' + str(cj_amount))
