...
from electrum.plugins.joinmarket.jmclient import (
    select, select_gradual, select_greedy, select_greediest,
    choose_orders, choose_sweep_orders, weighted_order_choose)
from electrum.plugins.joinmarket.jmclient.support import (
    calc_cj_fee, rand_exp_array, rand_norm_array, rand_weighted_choice,
...
class SupportTestCase(JMTestCase):

    async def test_utxo_selection(self):
        ...
        for selector in [select, select_gradual, select_greedy,
                         select_greediest]:
    ...
    async def test_random_funcs(self):
    ...
    async def test_calc_cjfee(self):
    ...
    async def test_choose_orders(self):
        jmman = self.jmman
        orderbook = copy.deepcopy(t_orderbook)
        # test not enough liquidity
        orders_fees = choose_orders(jmman, orderbook, 10000000, 7,
                                    weighted_order_choose)
        assert orders_fees == (None, 0)
        orders_fees = choose_orders(jmman, orderbook, 10000000, 3,
                                    weighted_order_choose)
        ...
        orders_fees = choose_orders(jmman, orderbook, 10000000, 1,
                                    weighted_order_choose)
        assert len(orders_fees[0]) == 1
        # test the hated 'cheapest'
        orders_fees = choose_orders(jmman, orderbook, 100000000, 3,
        ...
        orders_fees = choose_orders(
            jmman, orderbook, 100000000, 3,
            fidelity_bond_weighted_order_choose)
        assert len(orders_fees[0]) == 3
        # test sweep
        result, cjamount, total_fee = choose_sweep_orders(
            jmman, orderbook, 50000000, 30000, 3, weighted_order_choose, None)
        ...
        # test not enough liquidity
        result, cjamount, total_fee = choose_sweep_orders(
            jmman, orderbook, 50000000, 30000, 7, weighted_order_choose, None)
        ...
        orderbook.append({'counterparty': 'fake',
                          'ordertype': 'sw0absoffer', 'oid': 0,
                          'minsize': 7500000, 'txfee': 1000,
                          'maxsize': 599972700, 'cjfee': 9000})
        result, cjamount, total_fee = choose_sweep_orders(
            jmman, orderbook, 50000000, 30000, 7, cheapest_order_choose, None)
        assert total_fee > 0
        # (b)
        orderbook.append({'counterparty': 'fake2',
                          'ordertype': 'dummyoffer', 'oid': 0,
                          'minsize': 7500000, 'txfee': 1000,
                          'maxsize': 599972700, 'cjfee': 9000})
        result, cjamount, total_fee = choose_sweep_orders(
            jmman, orderbook, 50000000, 30000, 8, weighted_order_choose, None)
        # (c)
        # remove bad offer
        ...
        result, cjamount, total_fee = choose_sweep_orders(
            jmman, orderbook, 50000000, 30000, 4, weighted_order_choose, None)
        assert result is None
        assert cjamount == 0
        assert total_fee == 0
